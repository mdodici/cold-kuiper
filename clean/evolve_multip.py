...
if track_at_step == True:
    ...
    ts_track = np.append(0,np.logspace(0,np.log10(t_tot),num_track))
    N_track = np.zeros((num_track+1,gpe,gps))
    Nitrack = np.zeros((num_track+1,gpe,gps))
    Notrack = np.zeros((num_track+1,gpe,gps))
...
def save_files(N, N_i, N_o, ts, dt, fu, ts_end, it):
    ...
    if track_at_step == True:
        ...
        len_of_tracked = num_track+1
        
    np.save('%s/N.npy' %newdir, N.reshape(len_of_tracked,gpe*gps))
    np.save('%s/N_i.npy' %newdir, N_i.reshape(len_of_tracked,gpe*gps))
    np.save('%s/N_o.npy' %newdir, N_o.reshape(len_of_tracked,gpe*gps))
    np.save('%s/ts.npy' %newdir, ts)
    np.save('%s/dt.npy' %newdir, dt)
    np.save('%s/fu.npy' %newdir, fu)
    #np.save('%s/nrem.npy' %newdir, nrem_all)
...
try:  
    ...
    while it < ts_max - 1:
        ...
        if (it - 1) % 10 == 0:    # start a timer if iteration number ends in 1
            loop_start_time = time.time()   # this will run over next ten steps
        ...
        if ts[it] > t_tot:              # if this step was beyond the planned total time of integration, 
            ...
            print("Integration complete. Total t = %1.2E yr" %ts[it])
            print("%i steps taken. Total computing time: %f minutes" %(it, total_comp_time/60))
            break                       #    stop
        ...
        if it % 10 == 0:                # if this was first or n*10th step,
            if it == 0:                 #    print message for 1st step
                ...
                loop_computing_time = time.time() - loop_start_time
                step_avg_time = loop_computing_time/10    # calculate average time of last 10 int steps
                print("Step",it,"complete. t = %1.2E yr. Average step computing time %1.2f s." %(ts[it],step_avg_time))
